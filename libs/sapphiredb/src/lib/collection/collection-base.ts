...
import {startWithObservable} from '../helper/rxjs-helper';
...
export abstract class CollectionBase<T, Y> {
  ...
  constructor(protected collectionName: string,
    ...
    if (classType) {
      const primaryKeys = DecoratorHelper.getPrimaryKeys(this.classType as ClassType<T>);
  ...
  public snapshot(): Observable<Y> {
    ...
    return <Observable<Y>>this.connectionManagerService.sendCommand<QueryResponse>(queryCommand).pipe(
      map((response: QueryResponse) => response.result),
      tap((state) => {
        if (this.offlineManager) {
          this.serverState = state as Y;
          this.offlineManager.setState(this.collectionName, this.prefilters, state);
        }
      }),
      startWithObservable(startWithValue$),
      switchMap((state) => {
        if (!this.offlineManager) {
          return of(state);
        }
      ...
      map((result: Y) => {
        if (this.classType && this.classTransformer) {
          return this.classTransformer.plainToClass(result, this.classType) as unknown as Y;
        }
  ...
  public add(...values: T[]): Observable<CreateRangeResponse|OfflineResponse> {
    ...
    const result = this.offlineManager ? this.offlineManager.sendCommand<CreateRangeResponse, OfflineResponse>(command, this.primaryKeys) :
      this.connectionManagerService.sendCommand<CreateRangeResponse>(command);
  ...
  public remove(...values: Partial<T>[]): Observable<DeleteRangeResponse|OfflineResponse> {
    ...
    const primaryKeyList = values.map((value) => {
      const primaryValues: { [key: string]: unknown } = {};
      this.primaryKeys.forEach(pk => {
        primaryValues[pk] = (value as {[key: string]: unknown})[pk];
      });
      primaryValues['modifiedOn'] = (value as {[key: string]: unknown})['modifiedOn'];
    ...
    const result$ = this.offlineManager ?
      this.offlineManager.sendCommand<DeleteRangeResponse, OfflineResponse>(command, this.primaryKeys) :
      this.connectionManagerService.sendCommand<DeleteRangeResponse>(command);
  ...
  public invoke<TResult>(value: Partial<T>, action: string, ...parameters: any[]): Observable<TResult> {
    ...
    const primaryKeys: {[key: string]: unknown} = {};
    this.primaryKeys.forEach(pk => {
      primaryKeys[pk] = (value as {[key: string]: unknown})[pk];
    });
    primaryKeys['modifiedOn'] = (value as {[key: string]: unknown})['modifiedOn'];

    const command: InvokeCommand = new InvokeCommand(this.collectionName, action, primaryKeys, parameters);

    const result$ = this.connectionManagerService.sendCommand<InvokeResponse>(command, true);
  ...
  private createValuesSubscription(): CollectionValue<Y> {
    ...
    const wsSubscription = this.connectionManagerService.sendCommand<QueryResponse | ChangeResponse | ChangesResponse>(subscribeCommand, true)
      .subscribe((response: (QueryResponse | ChangeResponse | ChangesResponse)) => {
        if (response.responseType === 'QueryResponse') {
          collectionValue.subject.next({
            values: (response as QueryResponse).result,
